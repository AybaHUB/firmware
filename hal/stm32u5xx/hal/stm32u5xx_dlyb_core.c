...
The delay is Voltage and Temperature dependent, which may require FW to do re-tuning and recenter the Output clock phase
of the receive data.
...
- Set the output clock phase to a desired value using the DLYB_SetOutputClockPhase(), this value should not exceed
...
@note : The output clock phase value should not exceed the value returned by DLYB_CalculateMaxOutputClockPhase().
  */
/**
