...
/** @defgroup FMC_Byte_Lane FMC Byte Lane(NBL) Setup
  * @{
  */
#define FMC_BYTE_LANE_0AHB_CYCLE 0x00000000U
#define FMC_BYTE_LANE_1AHB_CYCLE FMC_BCRx_NBLSET_0
#define FMC_BYTE_LANE_2AHB_CYCLE FMC_BCRx_NBLSET_1
#define FMC_BYTE_LANE_3AHB_CYCLE FMC_BCRx_NBLSET_2
/**
  * @}
  */
...
/**
  * @brief  Convert the timing parameters from AHB clock cycle number to nanoseconds.
  * @param  timing_cycle  The timing parameter in AHB clock cycle number.
  * @retval timing parameter in nanoseconds.
  */
__STATIC_INLINE uint32_t FMC_NOR_SRAM_GetTimingNsec(uint32_t timing_cycle)
{
  return ((uint32_t)(((uint64_t)timing_cycle * (uint64_t)1000000000) / HAL_RCC_GetHCLKFreq()));
}
...
{
  return ((uint32_t)((((((uint32_t)instance - (uint32_t)FMC_Bank1_R) % 8UL) * 0x04000000UL) + FMC_BASE) +
                     (2U * (uint32_t)(offset))));
}
...
/**
  * @brief  Convert the voltage supply value to mv .
  * @param  voltage_value voltage value to convert to mv.
  * @retval The converted voltage value.
  */
__STATIC_INLINE uint32_t FMC_NOR_Get_Voltage_Info(uint16_t voltage_value)
{
  return ((uint32_t)(((voltage_value & 0x0FUL) * 100UL) + (((voltage_value & 0xF0UL) >> 4U) * 1000UL)));
}
/**
  * @}
  */
