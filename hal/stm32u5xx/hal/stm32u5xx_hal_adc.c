...
- ADC configuration
    ...
    - ADC peripheral is structured in subblocks with each a specific scope.
      HAL ADC follows this structure with a configuration structure and associated function for each subblock.
        ...
        - Optional subblocks
          ...
          - Oversampling
          - Offset
          - Multimode (prefix MULTI): encompass multiple ADC instances (one master, some slaves) for synchronized
            operation.
    - ADC instances may belong to an ADC common instance, in this case they may share features (clock configuration,
  ...
  - HAL ADC configuration steps:
    ...
    3. Case of multiple ADC instances used: Link HAL ADC handles using @ref HAL_ADC_SetLinkNextHandle()
...
#define ADC_MULTI_INDEX_MASTER          (0U) /*!< Multimode ADC instance master index
                                        in HAL ADC handle table "common" */
#define ADC_MULTI_INDEX_SLAVE           (1U) /*!< Multimode ADC instance slave index
                                        in HAL ADC handle table "common" */
#define ADC_MULTI_INST_COUNT            (2U) /*!< Multimode ADC instances count */
#endif /* ADC_MULTIMODE_SUPPORT */

#define ADC_GAIN_COMPENSATION_VAL_UNIT  (1000UL) /*!< HAL gain compensation value corresponding to unitary gain */
#define ADC_GAIN_COMPENSATION_VAL_MAX   (4096UL) /*!< HAL gain compensation value max */
...
#define ADC_STATE_ALL                   ((uint32_t)HAL_ADC_STATE_RESET | (uint32_t)HAL_ADC_STATE_INIT \
                                         | (uint32_t)HAL_ADC_STATE_CONFIG | (uint32_t)HAL_ADC_STATE_BUSY \
                                         | (uint32_t)HAL_ADC_STATE_IDLE | (uint32_t)HAL_ADC_STATE_ACTIVE) /*!< HAL ADC
                                         all global states */

#define ADC_STATE_ALL_EXCEPT_RESET      ((uint32_t)HAL_ADC_STATE_INIT \
                                         | (uint32_t)HAL_ADC_STATE_CONFIG | (uint32_t)HAL_ADC_STATE_BUSY \
                                         | (uint32_t)HAL_ADC_STATE_IDLE | (uint32_t)HAL_ADC_STATE_ACTIVE) /*!< HAL ADC
                                         all global states except reset */
/**
  * @}
  */
...
#define IS_ADC_INJ_TRIGGER_SRC_ADC12(trigger_src) (((trigger_src) == HAL_ADC_INJ_TRIG_SOFTWARE_START) || \
                                                   ...
                                                   ((trigger_src) == HAL_ADC_INJ_TRIG_FROM_GRP_REGULAR))
...
#define IS_ADC_CHANNEL(instance, trigger_src) (((instance) == HAL_ADC4) ? \
                                               IS_ADC_CHANNEL_ADC4(trigger_src) :\
                                               IS_ADC_CHANNEL_ADC12(trigger_src))
...
#define IS_ADC_CHANNEL_ENDED_MODE(single_diff) (((single_diff) == HAL_ADC_CHANNEL_SINGLE_ENDED) || \
                                                ((single_diff) == HAL_ADC_CHANNEL_DIFFERENTIAL))

/*! Assert definitions of ADC channel ending mode (specific ADC4) */
#define IS_ADC_CHANNEL_ENDED_MODE_ADC4(single_diff) (((single_diff) == HAL_ADC_CHANNEL_SINGLE_ENDED))
...
#define IS_ADC_GAIN_COMPENSATION(gain_compensation) ((1UL <= (gain_compensation)) && ((gain_compensation) <= 3999UL))
...
#define IS_ADC_AWD_INSTANCE(awd_instance) (((awd_instance) == HAL_ADC_AWD1) || \
                                           ((awd_instance) == HAL_ADC_AWD2) || \
                                           ((awd_instance) == HAL_ADC_AWD3))
...
#define IS_ADC_AWD_THRESHOLD(threshold) ((threshold) <= 0x01FFFFFFUL)

/*! Assert definitions of ADC analog watchdog threshold value (specific ADC4) */
#define IS_ADC_AWD_THRESHOLD_ADC4(threshold) ((threshold) <= 0xFFFUL)
...
#define IS_ADC_OVS_INSTANCE(ovs_instance) ((ovs_instance) == HAL_ADC_OVS1)

/*! Assert definitions of ADC oversampling scope */
#define IS_ADC_OVS_SCOPE(scope) (((scope) == HAL_ADC_OVS_DISABLE) || \
                                 ((scope) == HAL_ADC_OVS_GRP_REGULAR_CONTINUED) || \
                                 ((scope) == HAL_ADC_OVS_GRP_REGULAR_RESUMED) || \
                                 ((scope) == HAL_ADC_OVS_GRP_INJECTED) || \
                                 ((scope) == HAL_ADC_OVS_GRP_INJ_REG_RESUMED))
...
#define IS_ADC_OVS_DISCONT(discont) (((discont) == HAL_ADC_OVS_REG_CONT) || \
                                     ((discont) == HAL_ADC_OVS_REG_DISCONT))
...
#define IS_ADC_OFFSET_INSTANCE(offset_instance) (((offset_instance) == HAL_ADC_OFFSET1) || \
                                                 ((offset_instance) == HAL_ADC_OFFSET2) || \
                                                 ((offset_instance) == HAL_ADC_OFFSET3) || \
                                                 ((offset_instance) == HAL_ADC_OFFSET4))

/*! Assert definitions of ADC offset level value */
#define IS_ADC_OFFSET_LEVEL(level) ((level) <= 0x01FFFFFFUL)

/*! Assert definitions of ADC offset sign */
#define IS_ADC_OFFSET_SIGN(sign) (((sign) == HAL_ADC_OFFSET_SIGN_NEGATIVE) || \
                                  ((sign) == HAL_ADC_OFFSET_SIGN_POSITIVE))
...
#define IS_ADC_EVENT(event) (((event) == HAL_ADC_EVENT_EOC)   || \
                             ...
                             ((event) == HAL_ADC_EVENT_AWD1)  || \
                             ((event) == HAL_ADC_EVENT_AWD2)  || \
                             ((event) == HAL_ADC_EVENT_AWD3))
...
#if defined(ADC_MULTIMODE_SUPPORT)
static void adc_assert_state_multi_inst(const hal_adc_handle_t *hadc,
                                        const hal_adc_common_state_t common_state_expected,
                                        const hal_adc_state_t instance_state_expected);
static void adc_assert_state_multi_reg(const hal_adc_handle_t *hadc,
                                       const hal_adc_group_state_t group_state_expected);
static void adc_assert_state_multi_inj(const hal_adc_handle_t *hadc,
                                       const hal_adc_group_state_t group_state_expected);
static void adc_multi_set_state_inst(hal_adc_handle_t *hadc,
                                     hal_adc_common_state_t common_state,
                                     hal_adc_state_t instance_state);
static void adc_multi_set_state_inst_reg(hal_adc_handle_t *hadc,
                                         hal_adc_common_state_t common_state,
                                         hal_adc_state_t instance_state,
                                         hal_adc_group_state_t group_state);
static void adc_multi_set_state_inst_inj(hal_adc_handle_t *hadc,
                                         hal_adc_common_state_t common_state,
                                         hal_adc_state_t instance_state,
                                         hal_adc_group_state_t group_state);
static hal_status_t adc_multi_check_set_state_group(hal_adc_handle_t *hadc,
                                                    uint8_t group_index,
...
hal_status_t HAL_ADC_Init(hal_adc_handle_t *hadc, hal_adc_t instance)
{
  ...
  hadc->link_next_handle = (hal_adc_handle_t *) NULL;
  ...
  hadc->p_reg_end_of_sampl_cb = HAL_ADC_REG_EndOfSamplingCallback;
...
void HAL_ADC_DeInit(hal_adc_handle_t *hadc)
{
  ...
  ASSERT_DBG_STATE(hadc->global_state, ADC_STATE_ALL);
  ...
  if (hadc->link_next_handle != NULL)
  {
    /* Remove handle from daisy chain: Parse handles through links of daisy chain until loop back to previous handle */
    uint32_t index = ADC_MULTI_INST_COUNT; /* Maximum number of linked handles (to prevent infinite loop in case of
                                              pointer issue. In this case, daisy chain not updated.) */
    hal_adc_handle_t *handle_current = hadc;
    while (index != 0U)
    {
      if (handle_current->link_next_handle == hadc)
      {
        if (handle_current == hadc->link_next_handle)
        {
          ...
          handle_current->link_next_handle = NULL;
        }
        else
        {
          /* Daisy chain update: Set link from previous handle to next handle */
          handle_current->link_next_handle = hadc->link_next_handle;
        }
        hadc->link_next_handle = NULL;
        break;
      }
      else
      {
        /* Go to next handle */
        ASSERT_DBG_PARAM(handle_current->link_next_handle != NULL);
        handle_current = handle_current->link_next_handle;
      }
      index--;
    }
  }
...
/**
  ...
  *         Refer to device reference manual of ADC LL driver macro "LL_ADC_COMMON_INSTANCE()" returning
...
hal_status_t HAL_ADC_SetLinkNextHandle(hal_adc_handle_t *hadc_a, hal_adc_handle_t *hadc_b)
{
  ...
  ASSERT_DBG_PARAM(hadc_b->link_next_handle == NULL);

  /* Check whether selected ADC instances belong to the same ADC common instance */
  ASSERT_DBG_PARAM(LL_ADC_COMMON_INSTANCE(ADC_GET_INSTANCE(hadc_a))
                   == LL_ADC_COMMON_INSTANCE(ADC_GET_INSTANCE(hadc_b)));

  /* Check whether selected ADC instance is featuring ADC common instance with other ADC instances.
     Note: On STM32U5, only ADC1 and ADC2 (not available on all devices) have multiple instances per common instance */
  ASSERT_DBG_PARAM(ADC_GET_INSTANCE(hadc_a) != ADC4);
  ...
  ASSERT_DBG_STATE(hadc_b->common_state,
                   (uint32_t)HAL_ADC_COMMON_STATE_RESET |
                   (uint32_t)HAL_ADC_COMMON_STATE_INDEPT);
  ASSERT_DBG_STATE(hadc_a->global_state, ADC_STATE_ALL_EXCEPT_RESET);
  ASSERT_DBG_STATE(hadc_b->global_state, ADC_STATE_ALL_EXCEPT_RESET);

  if (hadc_a->link_next_handle == NULL)
  {
    /* First link */
    hadc_b->link_next_handle = hadc_a;
  }
  else
  {
    /* Additional link */
    hadc_b->link_next_handle = hadc_a->link_next_handle;
  }
  hadc_a->link_next_handle = hadc_b;
...
hal_status_t HAL_ADC_REG_SetDMA(hal_adc_handle_t *hadc, hal_dma_handle_t *hdma)
{
  ...
  ASSERT_DBG_STATE(hadc->global_state, ADC_STATE_ALL_EXCEPT_RESET);
...
hal_status_t HAL_ADC_SetConfig(hal_adc_handle_t *hadc, const hal_adc_config_t *p_config)
{
  ...
  if (LL_ADC_GetCommonClock(LL_ADC_COMMON_INSTANCE(p_instance)) != (uint32_t)p_config->clock)
  {
    ...
    {
      LL_ADC_SetCommonClock(LL_ADC_COMMON_INSTANCE(p_instance), (uint32_t)p_config->clock);
    }
    else
    {
  ...
  {
    LL_ADC_REG_SetSamplingMode(p_instance, (uint32_t)p_config->sampling_mode);
    LL_ADC_SetTriggerFrequencyMode(p_instance, (uint32_t)p_config->trigger_freq_mode);
  }

  if (hadc->global_state == HAL_ADC_STATE_INIT)
  {
    hadc->global_state = HAL_ADC_STATE_CONFIG;
  }
...
void HAL_ADC_GetConfig(const hal_adc_handle_t *hadc, hal_adc_config_t *p_config)
{
  ...
  p_config->clock = (hal_adc_clock_t)LL_ADC_GetCommonClock(LL_ADC_COMMON_INSTANCE(p_instance));
  ...
  {
    p_config->sampling_mode = (hal_adc_sampling_mode_t) LL_ADC_REG_GetSamplingMode(p_instance);
  }
}
...
hal_status_t HAL_ADC_REG_SetConfig(hal_adc_handle_t *hadc, const hal_adc_reg_config_t *p_config)
{
  ...
  {
    if (LL_ADC_REG_GetSamplingMode(p_instance) != LL_ADC_REG_SAMPLING_MODE_NORMAL)
    {
      status = HAL_ERROR;
    }
  }
  ...
  if (hadc->instance == HAL_ADC4)
  {
    ...
    else
    {
      ...
      /* Reset group regular sequence */
      CLEAR_REG(p_instance->CHSELR);
    }
  }
  else
  {
  ...
  LL_ADC_REG_SetSequencerDiscont(p_instance, (uint32_t)p_config->sequencer_discont);
  LL_ADC_REG_SetContinuousMode(p_instance, (uint32_t)p_config->continuous);
  LL_ADC_REG_SetOverrun(p_instance, (uint32_t)p_config->overrun);
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_reg_config_t structure containing ADC group regular configuration.
  */
void HAL_ADC_REG_GetConfig(hal_adc_handle_t *hadc, hal_adc_reg_config_t *p_config)
{
  ADC_TypeDef *p_instance;
...
hal_status_t HAL_ADC_INJ_SetConfig(hal_adc_handle_t *hadc, const hal_adc_inj_config_t *p_config)
{
  ...
  if ((p_config->trigger_src != HAL_ADC_INJ_TRIG_SOFTWARE_START)
      && (p_config->trigger_src != HAL_ADC_INJ_TRIG_FROM_GRP_REGULAR))
  {
  ...
  LL_ADC_INJ_SetTriggerSource(p_instance, (uint32_t)p_config->trigger_src & ~LL_ADC_INJ_TRIG_FROM_GRP_REGULAR);

  if (p_config->trigger_src != HAL_ADC_INJ_TRIG_SOFTWARE_START)
  {
    if (p_config->trigger_src == HAL_ADC_INJ_TRIG_FROM_GRP_REGULAR)
    {
      LL_ADC_INJ_SetTrigAuto(p_instance, LL_ADC_INJ_TRIG_FROM_GRP_REGULAR);
    }
    else
    {
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_inj_config_t structure containing ADC group injected configuration.
  */
void HAL_ADC_INJ_GetConfig(hal_adc_handle_t *hadc, hal_adc_inj_config_t *p_config)
{
  ADC_TypeDef *p_instance;
                                      ...
                                      const hal_adc_channel_config_t *p_config)
{
  ...
  if (hadc->instance == HAL_ADC4)
  {
    ...
    ASSERT_DBG_PARAM(IS_ADC_CHANNEL_ENDED_MODE_ADC4(p_config->single_diff));
  }
  else
  {
    ...
    ASSERT_DBG_PARAM(IS_ADC_CHANNEL_ENDED_MODE(p_config->single_diff));
  }

  ASSERT_DBG_STATE(hadc->global_state,
                   (uint32_t)HAL_ADC_STATE_CONFIG |
                   (uint32_t)HAL_ADC_STATE_IDLE);
  if (hadc->global_state == HAL_ADC_STATE_CONFIG)
  {
    ASSERT_DBG_STATE(hadc->group_state[ADC_GROUP_TO_STATE(p_config->group)],
  ...
  LL_ADC_SetChannelSingleDiff(p_instance, (uint32_t)channel_update, (uint32_t)p_config->single_diff);
  ...
  if (LL_ADC_IS_CHANNEL_INTERNAL((uint32_t)channel))
  {
    ...
    {
      LL_ADC_SetCommonPathInternalChAdd(LL_ADC_COMMON_INSTANCE(p_instance), LL_ADC_PATH_INTERNAL_VREFINT);
    }
    else if (channel == HAL_ADC_CHANNEL_TEMPSENSOR)
    {
      LL_ADC_SetCommonPathInternalChAdd(LL_ADC_COMMON_INSTANCE(p_instance), LL_ADC_PATH_INTERNAL_TEMPSENSOR);
    }
    else if (channel == HAL_ADC_CHANNEL_VBAT)
    {
      LL_ADC_SetCommonPathInternalChAdd(LL_ADC_COMMON_INSTANCE(p_instance), LL_ADC_PATH_INTERNAL_VBAT);
    }
    else if (channel == HAL_ADC_CHANNEL_DAC1CH1)
    {
      LL_ADC_SetPathInternalChRem(p_instance, LL_ADC_PATH_INTERNAL_DAC1CH2);
    }
    else if (channel == HAL_ADC_CHANNEL_DAC1CH2)
    {
...
/**
  ...
  *           @ref HAL_ADC_GROUP_REGULAR_INJECTED.
  ...
  * @param  p_config Pointer to a hal_adc_channel_config_t structure containing ADC channel configuration.
  */
void HAL_ADC_GetConfigChannel(hal_adc_handle_t *hadc, hal_adc_channel_t channel,
                              hal_adc_channel_config_t *p_config)
{
  ...
  if (hadc->instance == HAL_ADC4)
  {
    ...
    else
    {
      ...
      if ((LL_ADC_REG_GetSequencerChannels(p_instance) & (uint32_t)channel) != 0UL)
      {
        p_config->group = HAL_ADC_GROUP_REGULAR;
      }
    }

    p_config->single_diff = (hal_adc_channel_ended_mode_t)HAL_ADC_CHANNEL_SINGLE_ENDED;
  }
  else
  {
    for (index = 1U; index <= sequencer_length; index++)
    ...
    if (LL_ADC_GetChannelSingleDiff(p_instance, (uint32_t)channel) == 0UL)
    {
      p_config->single_diff = (hal_adc_channel_ended_mode_t)HAL_ADC_CHANNEL_SINGLE_ENDED;
    }
    else
    {
      p_config->single_diff = (hal_adc_channel_ended_mode_t)HAL_ADC_CHANNEL_DIFFERENTIAL;
    }
  }
...
hal_status_t HAL_ADC_MM_SetConfig(hal_adc_handle_t *hadc, const hal_adc_mm_config_t *p_config)
{
  ...
  adc_assert_state_multi_inst(hadc,
                              (hal_adc_common_state_t)((uint32_t)HAL_ADC_COMMON_STATE_LINKED |
                                                       (uint32_t)HAL_ADC_COMMON_STATE_MM),
                              HAL_ADC_STATE_IDLE);

  p_instance = ADC_GET_INSTANCE(hadc);
  p_common_instance = LL_ADC_COMMON_INSTANCE(p_instance);

  /* Check whether HAL ADC handle is the one of ADC master */
  ASSERT_DBG_PARAM(LL_ADC_MULTI_INSTANCE_MASTER(p_instance) == p_instance);
  ...
  adc_multi_set_state_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_IDLE);
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_mm_config_t structure containing ADC multimode configuration.
  */
void HAL_ADC_MM_GetConfig(hal_adc_handle_t *hadc, hal_adc_mm_config_t *p_config)
{
  ADC_TypeDef *p_instance;
  ADC_Common_TypeDef *p_common_instance;
  ...
  adc_assert_state_multi_inst(hadc,
                              (hal_adc_common_state_t)((uint32_t)HAL_ADC_COMMON_STATE_LINKED |
                                                       (uint32_t)HAL_ADC_COMMON_STATE_MM),
                              (hal_adc_state_t)ADC_STATE_ALL_EXCEPT_RESET);
...
hal_status_t HAL_ADC_MM_REG_SetMultiDMA(hal_adc_handle_t *hadc, const uint8_t *p_data, uint32_t size_byte)
{
  ...
  uint32_t status_tmp = (uint32_t) status;
  status_tmp |= (uint32_t)HAL_DMA_RegisterXferErrorCallback(hdma, adc_reg_dma_data_transfer_error_callback);
  status_tmp |= (uint32_t)HAL_DMA_RegisterXferHalfCpltCallback(hdma, adc_reg_dma_data_transfer_half_callback);
  status_tmp |= (uint32_t)HAL_DMA_RegisterXferCpltCallback(hdma, adc_reg_dma_data_transfer_cplt_callback);

  if ((hal_status_t) status_tmp == HAL_OK)
  {
  ...
  LL_ADC_SetMultiDMATransfer(LL_ADC_COMMON_INSTANCE(p_instance), LL_ADC_MULTI_REG_DMA_EACH_ADC);
...
hal_status_t HAL_ADC_SetConfigPostProcessing(hal_adc_handle_t *hadc, const hal_adc_post_processing_config_t *p_config)
{
  ...
  else
  {
    ...
    if ((uint32_t)p_config->gain_compensation_x1000 == ADC_GAIN_COMPENSATION_VAL_UNIT)
    {
      LL_ADC_SetGainCompensation(p_instance, 0U);
    }
    else
    {
      ...
      LL_ADC_SetGainCompensation(p_instance,
                                 (uint32_t)((p_config->gain_compensation_x1000 * ADC_GAIN_COMPENSATION_VAL_MAX)
  ...
  else
  {
    ...
    else
    {
      ...
      p_config->gain_compensation_x1000 = (((gain_compensation * ADC_GAIN_COMPENSATION_VAL_UNIT)
                                            / ADC_GAIN_COMPENSATION_VAL_MAX) + 1UL);
    }
  }
}
...
hal_status_t HAL_ADC_SetConfigLowPower(hal_adc_handle_t *hadc, const hal_adc_low_power_config_t *p_config)
{
  ...
  if (hadc->instance == HAL_ADC4)
  {
    LL_ADC_SetLPModeAutoPowerOff(p_instance, (uint32_t)p_config->lp_auto_power_off);
    LL_ADC_SetLPModeAutonomousDPD(p_instance, (uint32_t)p_config->lp_autonomous_dpd);
  }
...
/**
  ...
  */
hal_adc_vref_protection_t HAL_ADC_GetVrefProtection(hal_adc_handle_t *hadc)
{
  ADC_TypeDef *p_instance;
                                       ...
                                       const hal_adc_awd_config_t *p_config)
{
  ...
  uint32_t threshold_high_res;
  uint32_t threshold_low_res;
  ...
  if (p_config->channel != HAL_ADC_CHANNEL_NONE)
  {
    ...
    if ((hadc->instance == HAL_ADC4) || (awd_instance != HAL_ADC_AWD1))
    {
  ...
  if (p_config->channel != HAL_ADC_CHANNEL_NONE)
  {
    /* Thresholds computation: convert from numerical value to ADC peripheral digital value depending on resolution */
    adc_resolution = LL_ADC_GetResolution(p_instance);
    threshold_high_res = LL_ADC_ANALOGWD_SET_THRESHOLD_RES(p_instance, adc_resolution, p_config->threshold_high);
    threshold_low_res = LL_ADC_ANALOGWD_SET_THRESHOLD_RES(p_instance, adc_resolution, p_config->threshold_low);
    ...
    if ((hadc->instance != HAL_ADC4) && (awd_instance == HAL_ADC_AWD1))
    {
      LL_ADC_SetAWDFilteringConfiguration(p_instance, (uint32_t)awd_instance, (uint32_t)p_config->filtering);
    }
  }
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_awd_config_t structure containing ADC analog watchdog configuration.
  */
void HAL_ADC_GetConfigAnalogWD(hal_adc_handle_t *hadc, hal_adc_awd_instance_t awd_instance,
                               hal_adc_awd_config_t *p_config)
{
  ...
  threshold_high = (uint32_t)LL_ADC_GetAnalogWDThresholds(p_instance, (uint32_t)awd_instance,
                                                          LL_ADC_AWD_THRESHOLD_HIGH);
  threshold_low = (uint32_t)LL_ADC_GetAnalogWDThresholds(p_instance, (uint32_t)awd_instance,
                                                         LL_ADC_AWD_THRESHOLD_LOW);
  adc_resolution = LL_ADC_GetResolution(p_instance);
  p_config->threshold_high = LL_ADC_ANALOGWD_GET_THRESHOLD_RES(p_instance, adc_resolution, threshold_high);
  p_config->threshold_low = LL_ADC_ANALOGWD_GET_THRESHOLD_RES(p_instance, adc_resolution, threshold_low);

  if (awd_instance == HAL_ADC_AWD1)
  {
    p_config->filtering = (hal_adc_awd_filtering_t)
                          LL_ADC_GetAWDFilteringConfiguration(p_instance, (uint32_t)awd_instance);
  }
  else
  {
...
/**
  ...
  * @param  awd_threshold_value Analog watchdog threshold value.
  *         This parameter must be a number between Min_Data = 0x0000 and Max_Data = 0x3FFF (ADC1, ADC2) or 0xFFF (ADC4)
  * @retval HAL_INVALID_PARAM Invalid parameter
  * @retval HAL_ERROR         Operation completed with error
  * @retval HAL_OK            Operation completed successfully
  */
hal_status_t HAL_ADC_SetAnalogWDThresholds(hal_adc_handle_t *hadc, hal_adc_awd_instance_t awd_instance,
                                           hal_adc_awd_threshold_sel_t awd_threshold_sel, uint32_t awd_threshold_value)
{
  hal_status_t status = HAL_OK;
  ADC_TypeDef *p_instance;
  uint32_t adc_resolution;
  uint32_t threshold_value_res;
  ...
  /* Thresholds computation: convert from numerical value to ADC peripheral digital value depending on resolution */
  adc_resolution = LL_ADC_GetResolution(p_instance);
  threshold_value_res = LL_ADC_ANALOGWD_SET_THRESHOLD_RES(p_instance, adc_resolution, awd_threshold_value);
...
/**
  ...
  * @param  awd_instance Analog watchdog instance.
  * @param  awd_threshold_sel Analog watchdog threshold selection (high or low)
  * @retval threshold value (number between Min_Data = 0x0000 and Max_Data = 0x3FFF (ADC1, ADC2) or 0xFFF (ADC4))
  */
uint32_t HAL_ADC_GetAnalogWDThresholds(hal_adc_handle_t *hadc, hal_adc_awd_instance_t awd_instance,
                                       hal_adc_awd_threshold_sel_t awd_threshold_sel)
{
  ...
  /* Thresholds computation: convert from ADC peripheral digital value to numerical value depending on resolution */
  threshold_value = (uint32_t)LL_ADC_GetAnalogWDThresholds(p_instance, (uint32_t)awd_instance,
                                                           (uint32_t)awd_threshold_sel);
  adc_resolution = LL_ADC_GetResolution(p_instance);
  threshold_value = LL_ADC_ANALOGWD_GET_THRESHOLD_RES(p_instance, adc_resolution, threshold_value);
...
/**
  ...
  */
hal_adc_channel_t HAL_ADC_GetAnalogWDChannel(hal_adc_handle_t *hadc, hal_adc_awd_instance_t awd_instance)
{
  ADC_TypeDef *p_instance;
  hal_adc_channel_t channel;
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_ovs_config_t structure containing ADC oversampling configuration.
  */
void HAL_ADC_GetConfigOverSampling(hal_adc_handle_t *hadc, hal_adc_ovs_instance_t ovs_instance,
                                     ...
                                     const hal_adc_offset_config_t *p_config)
{
  ...
  if (p_config->channel != HAL_ADC_CHANNEL_NONE)
  {
    ...
    ASSERT_DBG_PARAM(IS_ADC_OFFSET_SIGN(p_config->sign));
  ...
  else
  {
    ...
    adc_resolution = LL_ADC_GetResolution(p_instance);
    offset_level_res = LL_ADC_OFFSET_SET_LEVEL_RES(adc_resolution, p_config->level);

    LL_ADC_SetOffset(p_instance, (uint32_t)offset_instance, (uint32_t)p_config->channel, offset_level_res);
    LL_ADC_SetOffsetSign(p_instance, (uint32_t)offset_instance, (uint32_t)p_config->sign);
...
/**
  ...
  * @param  p_config Pointer to a hal_adc_offset_config_t structure containing ADC offset configuration.
  */
void HAL_ADC_GetConfigOffset(hal_adc_handle_t *hadc,  hal_adc_offset_instance_t offset_instance,
                             hal_adc_offset_config_t *p_config)
{
  ...
  {
    p_config->channel = (hal_adc_channel_t)((uint32_t)LL_ADC_DECIMAL_NB_TO_CHANNEL(channel));
  }

  /* Thresholds computation: convert from ADC peripheral digital value to numerical value depending on resolution */
  adc_resolution = LL_ADC_GetResolution(p_instance);
  offset_level_res = LL_ADC_GetOffsetLevel(p_instance, (uint32_t) offset_instance);
  p_config->level = LL_ADC_OFFSET_GET_LEVEL_RES(adc_resolution, offset_level_res);

  p_config->sign = (hal_adc_offset_sign_t)LL_ADC_GetOffsetSign(p_instance, (uint32_t) offset_instance);
...
/**
  ...
  * @param  hadc Pointer to a hal_adc_handle_t structure.
  * @param  offset_instance Offset instance.
  * @param  offset_level ADC offset level value (number between Min_Data=0x000 and Max_Data=0x1FFFFFF).
  * @retval HAL_INVALID_PARAM Invalid parameter
  * @retval HAL_ERROR         Operation completed with error
  * @retval HAL_OK            Operation completed successfully
  */
                                    ...
                                    uint32_t offset_level)
{
  ...
  /* Offset level computation: convert from numerical value to ADC peripheral digital value depending on resolution */
  adc_resolution = LL_ADC_GetResolution(p_instance);
  offset_level_res = LL_ADC_OFFSET_SET_LEVEL_RES(adc_resolution, offset_level);

  LL_ADC_SetOffset(p_instance, (uint32_t)offset_instance, channel, offset_level_res);
...
/**
  ...
  * @param  hadc Pointer to a hal_adc_handle_t structure.
  * @param  offset_instance Offset instance.
  * @retval ADC offset level value (number between Min_Data=0x000 and Max_Data=0x1FFFFFF).
  */
uint32_t HAL_ADC_GetOffsetLevel(hal_adc_handle_t *hadc, hal_adc_offset_instance_t offset_instance)
{
  ADC_TypeDef *p_instance;
  uint32_t adc_resolution;
  uint32_t offset_level_res;
  ...
  adc_resolution = LL_ADC_GetResolution(p_instance);
  offset_level_res = LL_ADC_GetOffsetLevel(p_instance, (uint32_t) offset_instance);
  offset_level_res = LL_ADC_OFFSET_GET_LEVEL_RES(adc_resolution, offset_level_res);

  return offset_level_res;
}
...
void HAL_ADC_IRQHandler(hal_adc_handle_t *hadc)
{
  ...
  const uint32_t flag_status = LL_ADC_READ_REG(p_instance, ISR);
  const uint32_t it_sources = LL_ADC_READ_REG(p_instance, IER);
  const uint32_t flag_status_masked = flag_status & it_sources; /* Logical AND between flags status and interrupts
                                                                   sources enabled (registers bitfields aligned) */
...
#if defined(USE_HAL_ADC_REGISTER_CALLBACKS) && (USE_HAL_ADC_REGISTER_CALLBACKS == 1)
    hadc->p_reg_end_of_sampl_cb(hadc);
#else
...
#if defined(USE_HAL_ADC_REGISTER_CALLBACKS) && (USE_HAL_ADC_REGISTER_CALLBACKS == 1)
    hadc->p_awd_out_window_cb(hadc, HAL_ADC_AWD1);
#else
    HAL_ADC_AnalogWD_OutOfWindowCallback(hadc, HAL_ADC_AWD1);
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
  }
...
#if defined(USE_HAL_ADC_REGISTER_CALLBACKS) && (USE_HAL_ADC_REGISTER_CALLBACKS == 1)
    hadc->p_awd_out_window_cb(hadc, HAL_ADC_AWD2);
#else
    HAL_ADC_AnalogWD_OutOfWindowCallback(hadc, HAL_ADC_AWD2);
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
  }
...
#if defined(USE_HAL_ADC_REGISTER_CALLBACKS) && (USE_HAL_ADC_REGISTER_CALLBACKS == 1)
    hadc->p_awd_out_window_cb(hadc, HAL_ADC_AWD3);
#else
    HAL_ADC_AnalogWD_OutOfWindowCallback(hadc, HAL_ADC_AWD3);
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
  }
}
...
__weak void HAL_ADC_ErrorCallback(hal_adc_handle_t *hadc)
{
  ...
  /* Note : This function should not be modified. When the callback is needed, function
...
hal_status_t HAL_ADC_RegisterRegEndOfSamplingCallback(hal_adc_handle_t *hadc, hal_adc_cb_t p_callback)
{
  ...
  hadc->p_reg_end_of_sampl_cb = p_callback;
...
/**
  ...
  *         @arg @ref HAL_ADC_REG_ERROR_TIMEOUT
  *         @arg @ref HAL_ADC_REG_ERROR_OVR
  *         @arg @ref HAL_ADC_REG_ERROR_DMA
  *         @arg @ref HAL_ADC_INJ_ERROR_TIMEOUT
  *         @arg @ref HAL_ADC_INJ_ERROR_CONTEXTS_OVF
  *         @arg @ref HAL_ADC_MM_ERROR
  */
...
hal_status_t HAL_ADC_Start(hal_adc_handle_t *hadc)
{
  ...
  HAL_CHECK_UPDATE_STATE(hadc, global_state, HAL_ADC_STATE_IDLE, HAL_ADC_STATE_ACTIVE);

  if (LL_ADC_INJ_GetTrigAuto(p_instance) == LL_ADC_INJ_TRIG_FROM_GRP_REGULAR)
  {
...
hal_status_t HAL_ADC_Calibrate(hal_adc_handle_t *hadc)
{
  ...
  HAL_CHECK_UPDATE_STATE(hadc, global_state, HAL_ADC_STATE_ACTIVE, HAL_ADC_STATE_BUSY);
...
/**
  ...
  * @param  hadc Pointer to a hal_adc_handle_t structure.
  * @param  calib Pointer to a hal_adc_calib_t structure containing calibration data.
  * @retval HAL_INVALID_PARAM Invalid parameter
  * @retval HAL_ERROR         Operation completed with error
  * @retval HAL_OK            Operation completed successfully
  */
hal_status_t HAL_ADC_GetCalibrationFactor(hal_adc_handle_t *hadc, hal_adc_calib_t *calib)
{
  hal_status_t status = HAL_OK;
  ADC_TypeDef *p_instance;
  uint32_t cnt;
  ...
  if (hadc->instance == HAL_ADC4)
  {
    calib->factors[0] = LL_ADC_GetCalibrationOffsetFactor(p_instance, LL_ADC_SINGLE_ENDED);
  }
  else
  {
    ...
    SET_BIT(p_instance->CALFACT, ADC_CALFACT_CAPTURE_COEF);

    for (cnt = 0UL; cnt <= 8UL; cnt++)
    {
      MODIFY_REG(p_instance->CR, (ADC_CR_CALINDEX), (cnt << ADC_CR_CALINDEX_Pos));
      /* Use a Data Memory Barrier instruction to avoid synchronization issues when accessing ADC registers */
      __DMB();
      calib->factors[cnt] = (uint32_t)(READ_BIT(p_instance->CALFACT2, ADC_CALFACT2_CALFACT_Msk));
    }
  }
...
/**
  ...
  * @retval HAL_OK            Operation completed successfully
  */
hal_status_t HAL_ADC_SetCalibrationFactor(hal_adc_handle_t *hadc, hal_adc_calib_t *calib)
{
  hal_status_t status = HAL_OK;
  ADC_TypeDef *p_instance;
  uint32_t cnt;
  ...
  if (hadc->instance == HAL_ADC4)
  {
    LL_ADC_SetCalibrationOffsetFactor(p_instance, LL_ADC_SINGLE_ENDED, calib->factors[0]);
  }
  else
  {
    /* Note: Bitfields ADC_CALFACT_LATCH_COEF and ADC_CALFACT_CAPTURE_COEF have property "wr1",
    ...
    for (cnt = 0UL; cnt <= 7UL; cnt++)
    {
      ...
      if (cnt == 7UL)
      {
        ...
        /* Note: WRITE_REG operation instead of MODIFY_REG due to ADC_CALFACT2_CALFACT width 32 bits
                 (entire register) */
        WRITE_REG(p_instance->CALFACT2, calib->factors[cnt] | ((calib->factors[cnt + 1UL] & 0xFF000000UL) >> 8UL));
      }
      else
      {
        /* Note: WRITE_REG operation instead of MODIFY_REG due to ADC_CALFACT2_CALFACT width 32 bits
                 (entire register) */
        WRITE_REG(p_instance->CALFACT2, calib->factors[cnt]);
      }
    }

    SET_BIT(p_instance->CALFACT, ADC_CALFACT_LATCH_COEF);
    CLEAR_BIT(p_instance->CR, ADC_CR_CALINDEX);
  }
...
/**
  ...
  * @note   This function can be used to iterate a conversion process on going (applicable with configuration
  *         one conversion per trigger event (typically, SW start and no continuous mode), initiated
  *         by HAL_ADC_REG_StartConv_IT() or HAL_ADC_REG_StartConv_DMA().
  *         Necessary condition: previous conversion must be completed (state HAL_ADC_GROUP_STATE_IDLE, ensured by
...
/**
  ...
  *         @arg @ref HAL_ADC_OPT_IT_AWD1
  *         @arg @ref HAL_ADC_OPT_IT_AWD2
  *         @arg @ref HAL_ADC_OPT_IT_AWD3
  * @retval HAL_ERROR         Operation completed with error
  * @retval HAL_OK            Operation completed successfully
  */
...
hal_status_t HAL_ADC_REG_StopConv_DMA(hal_adc_handle_t *hadc)
{
  ...
  {
    status = HAL_DMA_RegisterXferAbortCallback(hadc->hdma_reg, adc_reg_dma_data_transfer_stop_callback);
...
hal_status_t HAL_ADC_REG_PollForConv(hal_adc_handle_t *hadc, uint32_t timeout_ms)
{
  ...
  ASSERT_DBG_STATE(hadc->global_state, HAL_ADC_STATE_ACTIVE);
  ASSERT_DBG_STATE(hadc->group_state[ADC_GROUP_REGULAR], HAL_ADC_GROUP_STATE_ACTIVE);
...
/**
  ...
  * @param  hadc Pointer to a hal_adc_handle_t structure.
  * @retval conversion data (number between Min_Data = 0x00000000 and Max_Data = 0xFFFFFFFF)
  */
uint32_t HAL_ADC_REG_GetValue(const hal_adc_handle_t *hadc)
{
  ADC_TypeDef *p_instance;

  ASSERT_DBG_PARAM(hadc != NULL);
  ASSERT_DBG_STATE(hadc->global_state, (hal_adc_state_t)ADC_STATE_ALL_EXCEPT_RESET);

  p_instance = ADC_GET_INSTANCE(hadc);
  uint32_t data = LL_ADC_REG_ReadConversionData32(p_instance);
...
/**
  ...
  * @note   This function can be used to iterate a conversion process on going (applicable with configuration
  *         one conversion per trigger event (typically, SW start and no auto trig from group regular), initiated
  *         by HAL_ADC_INJ_StartConv_IT().
  *         Necessary condition: previous conversion must be completed (state HAL_ADC_GROUP_STATE_IDLE, ensured by
...
hal_status_t HAL_ADC_INJ_StartConv(hal_adc_handle_t *hadc)
{
  ...
  ASSERT_DBG_PARAM(LL_ADC_INJ_GetTrigAuto(p_instance) != LL_ADC_INJ_TRIG_FROM_GRP_REGULAR);
...
/**
  ...
  * @param  it_opt Can be a combination of values (subset of @ref ADC_optional_interruptions):
  *         @arg @ref HAL_ADC_OPT_IT_NONE
  *         @arg HAL_ADC_OPT_IT_INJ_EOC
  *         @arg HAL_ADC_OPT_IT_INJ_EOS
  *         @arg HAL_ADC_OPT_IT_AWD1
  *         @arg @ref HAL_ADC_OPT_IT_AWD2
  *         @arg @ref HAL_ADC_OPT_IT_AWD3
  * @retval HAL_ERROR         Operation completed with error
  * @retval HAL_OK            Operation completed successfully
...
/**
  ...
  * @retval conversion data (number between Min_Data = 0x00000000 and Max_Data = 0xFFFFFFFF)
  */
uint32_t HAL_ADC_INJ_GetValue(const hal_adc_handle_t *hadc, uint8_t sequencer_rank)
{
  ADC_TypeDef *p_instance;
  uint32_t sequencer_rank_ll_format;
  ...
  uint32_t data = LL_ADC_INJ_ReadConversionData32(p_instance, sequencer_rank_ll_format);
...
hal_status_t HAL_ADC_MM_Start(hal_adc_handle_t *hadc)
{
  ...
  /* Check state of all HAL ADC handles part of multimode */
  adc_assert_state_multi_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_IDLE);

  adc_multi_set_state_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE);

  if (LL_ADC_INJ_GetTrigAuto(p_instance) == LL_ADC_INJ_TRIG_FROM_GRP_REGULAR)
  {
    status = adc_multi_check_set_state_group(hadc, ADC_GROUP_INJECTED,
                                             HAL_ADC_GROUP_STATE_IDLE,
                                             HAL_ADC_GROUP_STATE_ACTIVE);
  }
  ...
  handle_current = hadc;
  for (index = 0; index < ADC_MULTI_INST_COUNT; index++)
  {
...
hal_status_t HAL_ADC_MM_Stop(hal_adc_handle_t *hadc)
{
  ...
  /* Check state of all HAL ADC handles part of multimode */
  adc_assert_state_multi_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE);
  adc_assert_state_multi_reg(hadc, HAL_ADC_GROUP_STATE_IDLE);

  if (LL_ADC_INJ_GetTrigAuto(p_instance) == LL_ADC_INJ_TRIG_FROM_GRP_REGULAR)
  {
    adc_assert_state_multi_inj(hadc, (hal_adc_group_state_t)((uint32_t)HAL_ADC_GROUP_STATE_RESET |
                                                             (uint32_t)HAL_ADC_GROUP_STATE_IDLE |
                                                             (uint32_t)HAL_ADC_GROUP_STATE_ACTIVE));
  }
  else
  ...
  {
    hadc->group_state[ADC_GROUP_INJECTED] = HAL_ADC_GROUP_STATE_IDLE;
    adc_multi_set_state_inst_inj(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_IDLE, HAL_ADC_GROUP_STATE_IDLE);
  }
  else
  {
    adc_multi_set_state_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_IDLE);
...
hal_status_t HAL_ADC_MM_Calibrate(hal_adc_handle_t *hadc)
{
  ...
  adc_assert_state_multi_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE);
  adc_assert_state_multi_reg(hadc, HAL_ADC_GROUP_STATE_IDLE);
  adc_assert_state_multi_inj(hadc, HAL_ADC_GROUP_STATE_IDLE);

  /* Parse multimode instances through links of daisy chain (starting from ADC master) */
  handle_current = hadc;
  for (index = 0; index < ADC_MULTI_INST_COUNT; index++)
  {
    HAL_CHECK_UPDATE_STATE(handle_current, global_state, HAL_ADC_STATE_ACTIVE, HAL_ADC_STATE_BUSY);
...
/**
  ...
  * @note   This function can be used to iterate a conversion process on going (applicable with configuration
  *         one conversion per trigger event (typically, SW start and no continuous mode), initiated
  *         by HAL_ADC_MM_REG_StartConv_IT() or HAL_ADC_MM_REG_StartConv_DMA().
  *         Necessary condition: previous conversion must be completed (state HAL_ADC_GROUP_STATE_IDLE, ensured by
  *         polling (HAL_ADC_REG_PollForConv()) or interruption (HAL_ADC_IRQHandler()).
  * @warning  Prerequisite: HAL ADC handles part of multimode setup must have been linked using function
  *           @ref HAL_ADC_SetLinkNextHandle()
...
hal_status_t HAL_ADC_MM_REG_StartConv(hal_adc_handle_t *hadc)
{
  ...
  status = adc_multi_check_set_state_group(hadc, ADC_GROUP_REGULAR,
                                           HAL_ADC_GROUP_STATE_IDLE,
                                           HAL_ADC_GROUP_STATE_ACTIVE);
...
hal_status_t HAL_ADC_MM_REG_StartConv_IT_Opt(hal_adc_handle_t *hadc, uint32_t it_opt)
{
  ...
  p_instance_slv = ADC_GET_INSTANCE(hadc->link_next_handle);

  /* Check whether HAL ADC handle is the one of ADC master */
  ASSERT_DBG_PARAM(LL_ADC_MULTI_INSTANCE_MASTER(p_instance_mst) == p_instance_mst);
...
/**
  ...
  *         Default interruptions used: buffer half transfer and transfer complete. To use other interruptions,
  *         refer to HAL_ADC_MM_REG_StartConvM_DMA_Opt().
  * @note   Callback functions "HAL_ADC_REG_DataTransfer...Callback" (corresponding to these interruptions) will
  *         be triggered.
  * @warning  Prerequisite: HAL ADC handles part of multimode setup must have been linked using function
  *           @ref HAL_ADC_SetLinkNextHandle()
...
hal_status_t HAL_ADC_MM_REG_StartConvM_DMA_Opt(hal_adc_handle_t *hadc, uint32_t it_opt)
{
  ...
  if ((it_opt & (HAL_ADC_OPT_DMA_IT_HT | HAL_ADC_OPT_DMA_IT_DEFAULT)) != 0U)
  {
    ...
    LL_DMA_DisableIT((DMA_Channel_TypeDef *)((uint32_t)hadc->link_next_handle->hdma_reg->instance), LL_DMA_IT_ALL);
#if defined(HAL_ADC_OPT_DMA_IT_SILENT)
    if (hal_dma_opt_it != HAL_ADC_OPT_DMA_IT_SILENT)
#endif /* HAL_ADC_OPT_DMA_IT_SILENT */
    {
      ...
      LL_DMA_EnableIT((DMA_Channel_TypeDef *)((uint32_t)hadc->link_next_handle->hdma_reg->instance),
                      (LL_DMA_IT_TC  | LL_DMA_IT_DTE | LL_DMA_IT_ULE | LL_DMA_IT_USE | hal_dma_opt_it));
    }
  }
...
hal_status_t HAL_ADC_MM_REG_TrigNextConv(hal_adc_handle_t *hadc)
{
...
#if defined(USE_ASSERT_DBG_PARAM)
  uint32_t multimode = LL_ADC_GetMultimode(LL_ADC_COMMON_INSTANCE(p_instance_mst));
  ...
  /* Check state of all HAL ADC handles part of multimode */
  adc_assert_state_multi_reg(hadc, (hal_adc_group_state_t)
                             ((uint32_t)HAL_ADC_GROUP_STATE_IDLE |
#if defined(USE_HAL_ADC_DMA) && (USE_HAL_ADC_DMA == 1)
                              (uint32_t)HAL_ADC_GROUP_STATE_ACTIVE_SILENT |
#endif /* USE_HAL_ADC_DMA */
...
hal_status_t HAL_ADC_MM_REG_StopConv(hal_adc_handle_t *hadc)
{
  ...
  /* Check state of all HAL ADC handles part of multimode */
  adc_assert_state_multi_inst(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE);
  adc_assert_state_multi_reg(hadc, (hal_adc_group_state_t)((uint32_t)HAL_ADC_GROUP_STATE_IDLE |
                                                           (uint32_t)HAL_ADC_GROUP_STATE_ACTIVE));

  /* Multimode conversions controlled by ADC master only */
  LL_ADC_REG_StopConversion(p_instance_mst);
  ...
  adc_multi_set_state_inst_reg(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE, HAL_ADC_GROUP_STATE_IDLE);
...
hal_status_t HAL_ADC_MM_REG_StopConv_DMA(hal_adc_handle_t *hadc)
{
  ...
  if (hadc->link_next_handle->hdma_reg != NULL)
  {
    hadc_stop_last = hadc->link_next_handle;
    (void)HAL_DMA_Abort(hadc->hdma_reg);
  }
  else
  {
  ...
  {
    status = HAL_DMA_RegisterXferAbortCallback(hadc_stop_last->hdma_reg, adc_mm_reg_dma_data_transfer_stop_callback);
...
hal_status_t HAL_ADC_MM_REG_PollForConv(hal_adc_handle_t *hadc, uint32_t timeout_ms)
{
  ...
  if (hadc->group_conv_per_start[ADC_GROUP_REGULAR] == HAL_ADC_GROUP_CONV_UNIT)
  {
    ...
    hadc->link_next_handle->group_state[ADC_GROUP_REGULAR] = HAL_ADC_GROUP_STATE_IDLE;
  }
...
uint32_t HAL_ADC_MM_REG_GetValue(const hal_adc_handle_t *hadc, hal_adc_mm_inst_t multi_inst)
{
  ...
  ASSERT_DBG_PARAM(hadc != NULL);
  adc_assert_state_multi_inst(hadc, HAL_ADC_COMMON_STATE_MM, (hal_adc_state_t)ADC_STATE_ALL_EXCEPT_RESET);

  p_instance = ADC_GET_INSTANCE(hadc);

  ASSERT_DBG_PARAM(LL_ADC_GetMultiDMATransfer(LL_ADC_COMMON_INSTANCE(p_instance)) != LL_ADC_MULTI_REG_DMA_EACH_ADC);

  uint32_t data = LL_ADC_REG_ReadMultiConversionData32(LL_ADC_COMMON_INSTANCE(p_instance), (uint32_t)multi_inst);
...
/**
  ...
  * @note   This function can be used to iterate a conversion process on going (applicable with configuration
  *         one conversion per trigger event (typically, SW start and no auto trig from group regular), initiated
  *         by HAL_ADC_MM_INJ_StartConv_IT().
  *         Necessary condition: previous conversion must be completed (state HAL_ADC_GROUP_STATE_IDLE, ensured by
  *         polling (HAL_ADC_INJ_PollForConv()) or interruption (HAL_ADC_IRQHandler()).
  * @warning  Prerequisite: HAL ADC handles part of multimode setup must have been linked using function
  *           @ref HAL_ADC_SetLinkNextHandle()
...
hal_status_t HAL_ADC_MM_INJ_TrigNextConv(hal_adc_handle_t *hadc)
{
  ...
  /* Check state of all HAL ADC handles part of multimode */
  adc_assert_state_multi_inj(hadc, (hal_adc_group_state_t)((uint32_t)HAL_ADC_GROUP_STATE_IDLE |
                                                           (uint32_t)HAL_ADC_GROUP_STATE_ACTIVE));
...
hal_status_t HAL_ADC_MM_INJ_StopConv(hal_adc_handle_t *hadc)
{
  ...
  /* Multimode conversions controlled by ADC master only */
  LL_ADC_INJ_StopConversion(p_instance_mst);

  adc_multi_set_state_inst_inj(hadc, HAL_ADC_COMMON_STATE_MM, HAL_ADC_STATE_ACTIVE, HAL_ADC_GROUP_STATE_IDLE);
...
hal_status_t HAL_ADC_MM_INJ_StopConv_IT(hal_adc_handle_t *hadc)
{
  ...
  status = HAL_ADC_MM_INJ_StopConv(hadc);
...
hal_status_t HAL_ADC_MM_INJ_PollForConv(hal_adc_handle_t *hadc, uint32_t timeout_ms)
{
  ...
  if (hadc->group_conv_per_start[ADC_GROUP_INJECTED] == HAL_ADC_GROUP_CONV_UNIT)
  {
    hadc->group_state[ADC_GROUP_INJECTED] = HAL_ADC_GROUP_STATE_IDLE;
    /* ADC slave state machine update */
    hadc->link_next_handle->group_state[ADC_GROUP_INJECTED] = HAL_ADC_GROUP_STATE_IDLE;
  }
...
/**
  ...
  *         or raw data with ADC master and slave concatenated.
  * @note   Data width is limited to 16 bits (due to register 32 bits containing conversion data of two ADC instances).
  *         If expected data width is wider (this can be the case with features extending data width (oversampling,
  *         data shift,...), others services must be used:
  ...
  if (LL_ADC_IsEnabled(p_instance) == 0UL)
  {
    ...
    else
    {
      ...
      if ((LL_ADC_GetLPModeAutoPowerOff(p_instance) != LL_ADC_LP_AUTOPOWEROFF_ENABLE) || (hadc->instance != HAL_ADC4))
      {
        ...
        enabled_internal_channel = LL_ADC_GetCommonPathInternalCh(LL_ADC_COMMON_INSTANCE(p_instance));
        if ((enabled_internal_channel & (LL_ADC_PATH_INTERNAL_VREFINT | LL_ADC_PATH_INTERNAL_TEMPSENSOR)) != 0UL)
        {
          if ((enabled_internal_channel & LL_ADC_PATH_INTERNAL_VREFINT) != 0UL)
          {
...
hal_status_t adc_deactivate(hal_adc_handle_t *hadc)
{
     ...
     )
  {
    ...
    else
    {
      ...
      while ((p_instance->CR & ADC_CR_ADEN) != 0UL)
      {
        if ((HAL_GetTick() - tickstart) > ADC_DISABLE_TIMEOUT_MS)
        {
          /* New check to avoid false timeout detection in case of preemption */
          if ((p_instance->CR & ADC_CR_ADEN) != 0UL)
          {
            status = HAL_ERROR;
            break;
          }
    ...
    if (status == HAL_OK)
    {
      /* Set ADC instance to deepest disable level.
         Note: Except case of basic disable state needed by internal process (handle state busy). */
      if (hadc->global_state != HAL_ADC_STATE_BUSY)
      {
...
hal_status_t adc_calibrate(hal_adc_handle_t *hadc)
{
  ...
  if (status == HAL_OK)
  {
    ...
    if (hadc->instance == HAL_ADC4)
    {
      ...
      backup_setting_pwrr  = READ_BIT(p_instance->PWRR, ADC4_PWRR_AUTOFF);
      backup_setting_cfgr1 = READ_BIT(p_instance->CFGR1, ADC4_CFGR1_DMAEN | ADC4_CFGR1_DMACFG);
      CLEAR_BIT(p_instance->CFGR1, ADC4_CFGR1_DMAEN | ADC4_CFGR1_DMACFG);
      CLEAR_BIT(p_instance->PWRR, ADC4_PWRR_AUTOFF);
      ...
      SET_BIT(p_instance->CFGR1, backup_setting_cfgr1);
      SET_BIT(p_instance->PWRR, backup_setting_pwrr);
    }
    else /* ADC instance ADC1 or ADC2 */
    {
      /* Get device information */
      uint32_t dev_id = LL_SYSTEM_DBGMCU_GetDeviceID();
      uint32_t rev_id = LL_SYSTEM_DBGMCU_GetRevisionID();

      /* Assess whether extended calibration is available on the selected device */
      if ((dev_id == LL_SYSTEM_DBGMCU_DEV_ID_U535_U545) ||
          (dev_id == LL_SYSTEM_DBGMCU_DEV_ID_U5F5_U5G5) ||
          (((dev_id == LL_SYSTEM_DBGMCU_DEV_ID_U595_U5A5) || (dev_id == LL_SYSTEM_DBGMCU_DEV_ID_U575_U585))
           && (rev_id >= LL_SYSTEM_DBGMCU_REV_ID_C)))
      {
        ...
        {
          /* Use a Data Memory Barrier instruction to avoid synchronization issues when accessing ADC registers */
          MODIFY_REG(p_instance->CR, ADC_CR_CALINDEX, 0x9UL << ADC_CR_CALINDEX_Pos);
          __DMB();
          MODIFY_REG(p_instance->CALFACT2, 0xFFFFFF00UL, 0x03021100UL);
          __DMB();
          SET_BIT(p_instance->CALFACT, ADC_CALFACT_LATCH_COEF);

          status = adc_deactivate(hadc);

          MODIFY_REG(p_instance->CR, ADC_CR_ADCALLIN | ADC_CR_BITS_PROPERTY_RS, ADC_CR_ADCALLIN);

          MODIFY_REG(p_instance->CR, ADC_CR_BITS_PROPERTY_RS, ADC_CR_ADCAL);
...
/**
  ...
  *         using function @ref HAL_ADC_SetLinkNextHandle().
...
/**
  ...
  * @retval HAL_ERROR         ADC instance is already configured and can not be modified.
