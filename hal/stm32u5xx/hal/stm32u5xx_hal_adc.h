...
/** @addtogroup ADC_Private_Constants
  *  @{
  */
#define ADC_CALIB_FACTORS_BUF_SIZE      9U   /*!< ADC calibration factors buffer size */

#define ADC_GROUPS_COUNT                2U   /*!< HAL ADC handle internal tables size to manage ADC groups
...
#define HAL_ADC_REG_ERROR_TIMEOUT       (0x01UL << 4U)        /*!< ADC group regular conversion timeout */
#define HAL_ADC_REG_ERROR_OVR           (0x01UL << 5U)        /*!< ADC group regular overrun error */
#define HAL_ADC_REG_ERROR_DMA           (0x01UL << 6U)        /*!< ADC group regular DMA transfer error */

/* ADC group injected specific errors */
#define HAL_ADC_INJ_ERROR_TIMEOUT       (0x01UL << 8U)        /*!< ADC group injected conversion timeout */
#define HAL_ADC_INJ_ERROR_CONTEXTS_OVF  (0x01UL << 9U)        /*!< ADC group injected contexts queue overflow */

/* ADC multimode specific errors */
#define HAL_ADC_MM_ERROR                (0x01UL << 12U)       /*!< ADC multimode error (from either this ADC instance
                                                                   or other ADC instances part of multimode) */
...
#define HAL_ADC_TEMPERATURE_CALC_ERROR  LL_ADC_TEMPERATURE_CALC_ERROR /*!< Temperature calculation error using helper
                                                                           macro @ref HAL_ADC_CALC_TEMPERATURE(), due
                                                                           to issue on calibration parameters. This
                                                                           value is coded on 16 bits (to fit on
                                                                           signed word or double word) and
                                                                           corresponds to an inconsistent
                                                                           temperature value. */
/**
  * @}
  */
...
#define HAL_ADC_CHANNEL_TO_DECIMAL_NB(channel) \
  LL_ADC_CHANNEL_TO_DECIMAL_NB((channel))
...
/**
  ...
  * @param  adc_resolution_current Value among hal_adc_resolution_t
  * @param  adc_resolution_target Value among hal_adc_resolution_t
  * @retval ADC conversion data to the requested resolution
  */
#define HAL_ADC_CONVERT_DATA_RESOLUTION(data,                   \
                                        adc_resolution_current, \
                                        adc_resolution_target)  \
...
typedef enum
{
  HAL_ADC_STATE_RESET                   = 0,               /*!< ADC not yet initialized */
  HAL_ADC_STATE_INIT                    = (1U << 31U),     /*!< ADC initialized but not yet configured */
  HAL_ADC_STATE_CONFIG                  = (1U << 30U),     /*!< ADC configuration on going (step optional
                                                                  depending on series or ADC instances) */
  HAL_ADC_STATE_BUSY                    = (1U << 29U),     /*!< ADC internal process on going (calibration, ...) */
  HAL_ADC_STATE_IDLE                    = (1U << 28U),     /*!< ADC configured and ready to be activated */
  HAL_ADC_STATE_ACTIVE                  = (1U << 27U),     /*!< ADC activated, ready to operate
...
typedef enum
{
  HAL_ADC_LEFT_BIT_SHIFT_NONE           = LL_ADC_LEFT_BIT_SHIFT_NONE, /*!< ADC conversion data not shifted
                                          (alignment on data register MSB bit 0) */
...
typedef enum
{
  HAL_ADC_SAMPLING_MODE_NORMAL          = LL_ADC_REG_SAMPLING_MODE_NORMAL, /*!< ADC conversions sampling phase duration
                                          is defined using hal_adc_sampling_time_t. */
  HAL_ADC_SAMPLING_MODE_BULB            = LL_ADC_REG_SAMPLING_MODE_BULB,   /*!< ADC conversions sampling phase starts
                                          ...
                                          Note: Usable only if conversions from ADC group regular, not in continuous
                                                mode. */
  HAL_ADC_SAMPLING_MODE_TRIGGER_CTRL    = LL_ADC_REG_SAMPLING_MODE_TRIGGER_CTRL /*!< ADC conversions sampling phase
                                          is controlled by trigger events: Trigger rising edge = start sampling,
...
typedef enum
{
  ...
  HAL_ADC_INJ_TRIG_FROM_GRP_REGULAR     = LL_ADC_INJ_TRIG_FROM_GRP_REGULAR /*!< ADC group injected conversion trigger
                                          from ADC group regular end of sequence (no action ADC group injected
...
typedef enum
{
  ...
  HAL_ADC_INJ_SEQ_DISCONT_1RANK         = LL_ADC_INJ_SEQ_DISCONT_1RANK,   /*!< ADC group injected sequencer discontinuous
                                          mode enable with sequence interruption every rank */
...
typedef enum
{
  ...
  HAL_ADC_CHANNEL_DAC1CH2               = LL_ADC_CHANNEL_DAC1CH2_ADC4,    /*!< ADC channel to DAC1 channel 2,
                                          specific to ADC instance: ADC4 */
  HAL_ADC_CHANNEL_VDDCORE               = LL_ADC_CHANNEL_VCORE,            /*!< ADC channel to Vcore,
                                          specific to ADC instance: ADC4 */
...
typedef enum
{
  ...
  HAL_ADC_SAMPLING_TIME_814CYCLES       = LL_ADC_SAMPLINGTIME_814CYCLES,    /*!< Sampling time 814 ADC clock cycles
                                          (specific to ADC instance: ADC1, ADC2) */
  HAL_ADC_SAMPLING_TIME_COMMON_1        = LL_ADC_SAMPLINGTIME_COMMON_1,    /*!< Sampling time group 1
                                          (specific to ADC instance: ADC4) */
  HAL_ADC_SAMPLING_TIME_COMMON_2        = LL_ADC_SAMPLINGTIME_COMMON_2,    /*!< Sampling time group 2
                                          (specific to ADC instance: ADC4) */
} hal_adc_sampling_time_t;

/**
  * @brief HAL ADC channel ending (single or differential ended)
  */
typedef enum
{
  HAL_ADC_CHANNEL_SINGLE_ENDED          = LL_ADC_SINGLE_ENDED,          /*!< ADC channel ending set to single-ended */
  HAL_ADC_CHANNEL_DIFFERENTIAL          = LL_ADC_DIFFERENTIAL_ENDED,    /*!< ADC channel ending set to differential */
} hal_adc_channel_ended_mode_t;
...
typedef enum
{
  HAL_ADC_AWD1                          = LL_ADC_AWD1, /*!< ADC analog watchdog number 1 */
  HAL_ADC_AWD2                          = LL_ADC_AWD2, /*!< ADC analog watchdog number 2 */
  HAL_ADC_AWD3                          = LL_ADC_AWD3  /*!< ADC analog watchdog number 3 */
...
typedef enum
{
  HAL_ADC_AWD_THRESHOLD_HIGH            = LL_ADC_AWD_THRESHOLD_HIGH, /*!< ADC analog watchdog threshold high.
                                          ...
                                          and Max_Data = 0x3FFF (ADC1, ADC2) or 0xFFF (ADC4). */
...
typedef enum
{
  HAL_ADC_OVS1                          = 0U, /*!< ADC oversampling instance 1 */
...
typedef enum
{
  ...
  HAL_ADC_OVS_GRP_REGULAR_CONTINUED     = LL_ADC_OVS_GRP_REGULAR_CONTINUED, /*!< ADC oversampling on ADC group regular
                                          conversions.
                                          If group injected interrupts group regular: when ADC group injected is
                                          triggered, the oversampling on ADC group regular is temporary stopped
                                          and continued afterwards. */
  HAL_ADC_OVS_GRP_REGULAR_RESUMED       = LL_ADC_OVS_GRP_REGULAR_RESUMED,   /*!< ADC oversampling on ADC group regular
                                          conversions.
                                          If group injected interrupts group regular: when ADC group injected is
                                          triggered, the oversampling on ADC group regular is resumed from start
                                          (oversampler buffer reset).
                                          (specific to ADC instance: ADC1, ADC2) */
  HAL_ADC_OVS_GRP_INJECTED              = LL_ADC_OVS_GRP_INJECTED,          /*!< ADC oversampling on ADC group injected
                                          conversions.
                                          (specific to ADC instance: ADC1, ADC2) */
  HAL_ADC_OVS_GRP_INJ_REG_RESUMED       = LL_ADC_OVS_GRP_INJ_REG_RESUMED,   /*!< ADC oversampling on both ADC groups
...
typedef enum
{
  HAL_ADC_OVS_REG_CONT                  = LL_ADC_OVS_REG_CONT,   /*!< ADC oversampling discontinuous mode: continuous
                                          (all conversions of oversampling ratio are done from 1 trigger) */
  HAL_ADC_OVS_REG_DISCONT               = LL_ADC_OVS_REG_DISCONT /*!< ADC oversampling discontinuous mode: discontinuous
...
typedef enum
{
  HAL_ADC_OFFSET1                       = LL_ADC_OFFSET_1, /*!< ADC offset instance 1: ADC channel and offset level
  ...
  HAL_ADC_OFFSET2                       = LL_ADC_OFFSET_2, /*!< ADC offset instance 2: ADC channel and offset level
                                          to which the offset programmed will be applied (independently of channel
                                          assigned on ADC group regular or injected sequencer) */
  HAL_ADC_OFFSET3                       = LL_ADC_OFFSET_3, /*!< ADC offset instance 3: ADC channel and offset level
                                          to which the offset programmed will be applied (independently of channel
                                          assigned on ADC group regular or injected sequencer) */
  HAL_ADC_OFFSET4                       = LL_ADC_OFFSET_4, /*!< ADC offset instance 4: ADC channel and offset level
                                          to which the offset programmed will be applied (independently of channel
                                          assigned on ADC group regular or injected sequencer) */
} hal_adc_offset_instance_t;

/**
  * @brief HAL ADC offset sign
  */
typedef enum
{
  HAL_ADC_OFFSET_SIGN_NEGATIVE          = LL_ADC_OFFSET_SIGN_NEGATIVE, /*!< ADC offset sign negative */
  HAL_ADC_OFFSET_SIGN_POSITIVE          = LL_ADC_OFFSET_SIGN_POSITIVE  /*!< ADC offset sign positive */
} hal_adc_offset_sign_t;
...
typedef enum
{
  HAL_ADC_EVENT_EOC                     = LL_ADC_FLAG_EOC,        /*!< ADC event group regular end of unitary
                                          conversion */
  ...
  HAL_ADC_EVENT_JEOC                    = LL_ADC_FLAG_JEOC,       /*!< ADC event group injected end of unitary
                                          conversion */
  HAL_ADC_EVENT_JEOS                    = LL_ADC_FLAG_JEOS,       /*!< ADC event group injected end of sequence
                                          conversions */
  HAL_ADC_EVENT_AWD1                    = LL_ADC_FLAG_AWD1,       /*!< ADC event analog watchdog 1 */
  HAL_ADC_EVENT_AWD2                    = LL_ADC_FLAG_AWD2,       /*!< ADC event analog watchdog 2 */
  HAL_ADC_EVENT_AWD3                    = LL_ADC_FLAG_AWD3,       /*!< ADC event analog watchdog 3 */
} hal_adc_event_t;
...
#define HAL_ADC_OPT_IT_INJ_EOS            (LL_ADC_IT_JEOS)        /*!< ADC optional interruption group injected end of
                                          sequence conversions */
#define HAL_ADC_OPT_IT_AWD1               (LL_ADC_IT_AWD1)        /*!< ADC optional interruption analog watchdog 1
                                          out of window event */
#define HAL_ADC_OPT_IT_AWD2               (LL_ADC_IT_AWD2)        /*!< ADC optional interruption analog watchdog 2
                                          out of window event */
#define HAL_ADC_OPT_IT_AWD3               (LL_ADC_IT_AWD3)        /*!< ADC optional interruption analog watchdog 3
                                          out of window event */
...
typedef struct
{
  ...
  hal_adc_trigger_freq_mode_t       trigger_freq_mode;  /*!< ADC trigger frequency mode
                                                             (specific to ADC instance: ADC1, ADC2.
                                                              For ADC4, fixed to low frequency.) */
...
typedef struct
{
  ...
  uint32_t                          gain_compensation_x1000; /*!< ADC data post-processing: ADC gain coefficient applied
                                                                  to raw conversions data. Value multiplicated by 1000
                                                                  to have an integer value (example: to set
                                                                  coefficient 0.982, parameter value must be 982).
                                                                  Can be a number between: Min_Data = 1, Max_Data = 3999
                                                                  (specific to ADC instance: ADC1, ADC2).
                                                                  Note: Coefficient resolution managed by ADC peripheral
                                                                        is higher than 1/1000, if needed to reach more
                                                                        accurate resolution, refer to function
                                                                        LL_ADC_SetGainCompensation() */
} hal_adc_post_processing_config_t;

/**
  * @brief ADC instance configuration advanced: low power features
  * @note  On this STM32 series, ADC features autonomous operation (LPBAM), it can be enabled from RCC,
  *        refer to ref manual "peripheral clock enable in Sleep and Stop modes"
  *        (specific to ADC instance: ADC4).
  */
typedef struct
{
  ...
  hal_adc_lp_auto_power_off_state_t lp_auto_power_off;  /*!< ADC low power feature auto power-off.
                                                             Note: this feature can be combined with auto wait and
  ...
  hal_adc_lp_autonomous_dpd_state_t lp_autonomous_dpd;  /*!< ADC low power feature deep power down (DPD) mode during
                                                             autonomous operation (LPBAM).
                                                             Note: this feature is optional for autonomous operation,
                                                                   it allows to optimize power consumption (with
                                                                   constraint of additional latency delay
...
typedef struct
{
  uint32_t                          factors[ADC_CALIB_FACTORS_BUF_SIZE]; /*!< ADC calibration factors buffer.
...
typedef struct
{
  ...
  uint8_t                           sequencer_length;   /*!< ADC group regular sequencer scan length.
                                                             Note: Specificity with ADC instance ADC4: parameter
                                                                   conditioned to parameter "sequencer_mode" set
                                                                   to configurable (otherwise, length equal to
...
typedef struct
{
  ...
  uint8_t                           sequencer_rank;     /*!< ADC group sequencer rank (position in conversions sequence)
                                                             Note: Specificity with ADC instance ADC4: parameter
                                                                   conditioned to parameter "sequencer_mode" set
                                                                   to configurable (otherwise, rank fixed to
                                                                   channel number)
                                                             Can be a number in range of parameter "sequencer_length" in
                                                             structures hal_adc_reg_config_t and hal_adc_inj_config_t:
                                                             Min_Data = 1, Max_Data = {4; 8; 16} depending on ADC group
                                                             and ADC instance. */
  hal_adc_sampling_time_t           sampling_time;      /*!< Sampling time
                                                             Note: Sampling time scope depends on ADC instance: channel
                                                                   wise (ADC1, ADC2) or group of channels wise (ADC4) */
  hal_adc_channel_ended_mode_t      single_diff;        /*!< ADC channel single or differential ended */
...
typedef struct
{
  hal_adc_group_t                   group;              /*!< ADC group ADC group from which conversion data is compared
                                                             to thresholds.
                                                             Analog watchdog instances specificities impacting this
                                                             parameter: refer to description of
                                                             "hal_adc_awd_instance_t". */
  hal_adc_channel_t                 channel;            /*!< ADC channel to be monitored */
  uint32_t                          threshold_high;     /*!< ADC analog watchdog threshold high value */
  uint32_t                          threshold_low;      /*!< ADC analog watchdog threshold low value */
  hal_adc_awd_filtering_t           filtering;          /*!< ADC analog watchdog filtering
                                                             (specific to ADC instance: ADC1, ADC2
                                                             and specific to AWD instance: AWD1) */
...
typedef struct
{
  ...
  uint16_t                          ratio;              /*!< ADC oversampling ratio: sum of conversions data computed
                                                             to oversampling conversions data (before potential shift).
  ...
  uint8_t                           shift;              /*!< ADC oversampling shift: right bit shift to perform a
                                                             ...
                                                             Note: to compute shift value from ratio value and keeping
                                                                   data resolution, use function
...
typedef struct
{
  ...
  uint32_t                          level;              /*!< ADC offset level to be subtracted or added
                                                             to the raw ADC converted data.
                                                             Value between Min_Data=0x000 and Max_Data=0x1FFFFFF. */
  hal_adc_offset_sign_t             sign;               /*!< ADC offset sign */
...
typedef struct
{
  ...
  hal_adc_mm_reg_data_transfer_pack_t reg_data_transfer_packing; /*!< ADC multimode group regular data transfer
                                                             by DMA packing */
#endif /* USE_HAL_ADC_DMA */
  hal_adc_mm_interl_delay_t         interl_delay;       /*!< ADC multimode interleaved delay between two sampling phases
                                                             (parameter applicable only for mode interleaved) */
} hal_adc_mm_config_t;
...
struct hal_adc_handle_s
{
  ...
  hal_adc_handle_t *link_next_handle;   /*!< Pointer to another HAL ADC handle of instance belonging
                                             to the same ADC common instance (therefore, sharing common features).
  ...
  volatile hal_adc_group_state_t group_state[ADC_GROUPS_COUNT]; /*!< HAL ADC groups (regular, injected) state */
  volatile hal_adc_common_state_t common_state;                 /*!< HAL ADC handle link to common instance state */

  volatile hal_adc_group_conv_per_start_t group_conv_per_start[ADC_GROUPS_COUNT];  /*!< ADC group conversions per
  ...
  hal_adc_cb_t p_reg_end_of_sampl_cb;   /*!< Callback function: ADC group regular end of sampling phase */
  hal_adc_cb_t p_reg_eoc_cb;            /*!< Callback function: ADC group regular end of unitary conversion */
  hal_adc_cb_t p_reg_eos_cb;            /*!< Callback function: ADC group regular end of sequence conversions */
#if defined(USE_HAL_ADC_DMA) && (USE_HAL_ADC_DMA == 1)
  hal_adc_cb_t p_reg_xfer_half_cb;      /*!< Callback function: ADC group regular conv data buffer half transfer */
  hal_adc_cb_t p_reg_xfer_cplt_cb;      /*!< Callback function: ADC group regular conv data buffer transfer complete */
  hal_adc_cb_t p_reg_xfer_stop_cb;     /*!< Callback function: ADC group regular conv data transfer abort */
#endif /* USE_HAL_ADC_DMA */
  hal_adc_cb_t p_inj_eoc_cb;            /*!< Callback function: ADC group injected end of unitary conversion */
  hal_adc_cb_t p_inj_eos_cb;            /*!< Callback function: ADC group injected end of sequence conversions */
  hal_adc_awd_cb_t p_awd_out_window_cb; /*!< Callback function: ADC analog watchdog out of window event */
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
